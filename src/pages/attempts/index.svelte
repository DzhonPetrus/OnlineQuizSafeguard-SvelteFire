...
<div class="w-full bg-white p-12 mt-16 shadow-2xl rounded my-24 ">
    <div class="header flex items-end justify-between mb-12">
      ...
      <!--  <div class="text-end">
        ...
        <Collection path={`attempts`} query={ref => ref.where('email','==', $currentUser.email).limit(10)} let:data={attempts} let:ref={attemptsRef}>
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-12">


    {#each attempts as attempt}

        <Collection path={`quizzes`} query={ref => ref.where(firebase.firestore.FieldPath.documentId(), '==', attempt.quizId)} let:data={quizzes} >
            {#each quizzes as quiz}

            <div class="overflow-hidden shadow-lg rounded-lg h-90 w-60 md:w-80 cursor-pointer m-auto">
                <a href="{`/attempts/${attempt.id}`}" class="w-full block h-full">
                        ...
                        <div class="flex items-center mt-4">
                            <div class="flex flex-col justify-between ml-4 text-sm">
                                <p class="text-gray-800 dark:text-white">
                                    Attempted
                                    {humanizedFBDateTime(attempt.createdAt)}
                                </p>
